...
Users may discover {\fact}s by searching the data in the back-end. Their observations can be generated from data histograms and aggregated queries. For instance, the user might observe the drastic drop in the number of taxi trips on January 23, 2016 from the histogram in Figure \ref{fig:yellowstats}. 
...
Due to their drawbacks, we define a new metric, \textit{explanation index}, which balance out these two metrics according to the preferences of the user: 
...
It means that the example {\explanation}s are not convincing enough to explain the {\fact}. 
